...
const ClientTestimonialCard = () => {
  ...
  // If client already has a testimonial, don't render anything
  if (existing) return null;

  // Just submitted — show brief confirmation
  if (submitted) {
    return (
      <motion.div
    ...
    <>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground">Deja tu testimonio</p>
            <p className="text-[10px] text-muted-foreground">Aparecerá en nuestra web · no se puede editar</p>
          </div>
        </div>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent className="max-w-sm">
          <AlertDialogHeader>
            <AlertDialogTitle>¿Publicar testimonio?</AlertDialogTitle>
            <AlertDialogDescription>
              Una vez publicado no podrás editarlo ni eliminarlo. ¿Estás seguro de que quieres continuar?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
